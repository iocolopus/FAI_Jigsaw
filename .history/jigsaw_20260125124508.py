...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        # ESTRATEGIA 1: Alinear los extremos (las esquinas deben coincidir)
        # Alineamos el punto inicial y final de ambos contornos
        # El punto medio entre los extremos de cada contorno debería coincidir
        mid_x1 = (x1[0] + x1[-1]) / 2
        mid_y1 = (y1[0] + y1[-1]) / 2
        mid_x2 = (x2[0] + x2[-1]) / 2
        mid_y2 = (y2[0] + y2[-1]) / 2
        ...
        # ESTRATEGIA 2: Ajuste fino - probar pequeños desplazamientos en x e y
        # para encontrar la traslación que minimiza el error
        best_error = np.sum((x1 - x2) ** 2 + (y1 - y2) ** 2)
        best_tx, best_ty = 0, 0
        
        # Rango de búsqueda: ±10 píxeles con paso de 0.5
        search_range = np.arange(-10, 10.5, 0.5)
        for dtx in search_range:
            for dty in search_range:
                error = np.sum((x1 - (x2 + dtx)) ** 2 + (y1 - (y2 + dty)) ** 2)
                if error < best_error:
                    best_error = error
                    best_tx, best_ty = dtx, dty
