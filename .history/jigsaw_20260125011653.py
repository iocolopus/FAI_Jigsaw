    ...
    def solve(self):
        ...
        def r_solve(current_solution):
            ...
            if len(current_solution) < 15:
                ...
                required_edges = maping_dif_to_edge_id[dif]
                ...
                for pieza_index, pieza in enumerate(self.pieces):
                    ...
                    for rot in range(4):
                        ...
                        # Comprobamos si la arista coincide con la arista requerida
                        # Obtenemos la pieza y arista con la que debe coincidir
                        prev_piece_index, prev_rot = current_solution[-1]
                        prev_piece = self.pieces[prev_piece_index]
                        prev_edge = prev_piece.edges[prev_rot]

                        if edge.kind == prev_edge.kind:
                            continue  # no pueden ser del mismo tipo

                        # Calculamos disimilitud
                        dissim = edge.dissimilarity(prev_edge)

                        # Si la disimilitud es aceptable, añadimos a la solucion y seguimos recursivamente
                        if dissim < some_threshold:  # definir un umbral adecuado
                            new_solution = current_solution + [(pieza_index, rot)]
