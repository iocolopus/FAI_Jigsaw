...
class  Backtrack_solver():
    ...
    def solve(self):
        ...
        def r_solve(current_solution):
            ...
            for i, pieza in enumerate(self.pieces):
                if any(pieza_index == i for pieza_index, _ in current_solution):
                    continue  # Ya hemos usado esta pieza

                for rot in range(4):
                    edge = pieza.edges[rot]

                    # Comprobar compatibilidad con piezas adyacentes
                    compatible = True

                    # Comprobar pieza de arriba
                    if next_row > 0:
                        above_index, above_rot = current_solution[spiral_index.index((next_row - 1, next_col))]
                        above_piece = self.pieces[above_index]
                        above_edge = above_piece.edges[(above_rot + 2) % 4]
                        if edge.dissimilarity(above_edge) == np.inf:
                            compatible = False

                    # Comprobar pieza de la izquierda
                    if next_col > 0:
                        left_index, left_rot = current_solution[spiral_index.index((next_row, next_col - 1))]
                        left_piece = self.pieces[left_index]
                        left_edge = left_piece.edges[(left_rot + 1) % 4]
                        if edge.dissimilarity(left_edge) == np.inf:
                            compatible = False

                    if compatible:
                        new_solution = current_solution + [(i, rot)]
