...
class Piece:
    ...
    def generate_edges(self):
        ...
        for i in range(num_corners):
            ...
            end_corner = self.corners[(i + 1) % num_corners]
            mask = np.zeros((500, 500), dtype=np.uint8)
            cv.drawContours(mask, [self.contour], -1, 255, -1)
            cv.line(mask, tuple(start_corner), tuple(end_corner), 0, 5)
            edge_contour, _ = cv.findContours(mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
            if edge_contour:
                edges.append(Edge(edge_contour[0]))
            else:
                edges.append(Edge(np.array([])))
        self.edges = edges
