...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        Utiliza remuestreo uniforme y alineación óptima para encontrar la mínima distancia.
        ...
        # PASO 1: Normalizar ambos contornos para que empiecen en (0,0) y terminen en (L,0)
        # donde L es la longitud promedio entre ambos
        
        # Longitud de cada contorno (distancia entre extremos)
        len1 = x1[-1] - x1[0]
        len2 = x2[-1] - x2[0]
        target_len = (len1 + len2) / 2
        
        # Escalar y trasladar contorno 1: empieza en 0, escalar a target_len
        x1_norm = (x1 - x1[0]) * (target_len / len1) if len1 != 0 else x1 - x1[0]
        y1_norm = y1 - y1[0]
        
        # Escalar y trasladar contorno 2: empieza en 0, escalar a target_len  
        x2_norm = (x2 - x2[0]) * (target_len / len2) if len2 != 0 else x2 - x2[0]
        y2_norm = y2 - y2[0]
        
        # PASO 2: Ajuste fino en Y (la X ya está alineada por los extremos)
        best_error = np.sum((x1_norm - x2_norm) ** 2 + (y1_norm - y2_norm) ** 2)
        best_ty = 0
        
        # Buscar solo en Y ya que X está normalizada
        for dty in np.arange(-5, 5.1, 0.25):
            error = np.sum((x1_norm - x2_norm) ** 2 + (y1_norm - (y2_norm + dty)) ** 2)
            if error < best_error:
                best_error = error
                best_ty = dty
        
        y2_norm = y2_norm + best_ty
        
        # Calcular disimilitud como suma de diferencias cuadráticas
        mean_root_square_error = (np.sum((x1_norm - x2_norm) ** 2 + (y1_norm - y2_norm) ** 2) / n_samples) ** 0.5
        ...
        if plot:
            plt.plot(x1_norm, y1_norm, 'o-', label='Arista 1', linewidth=2)
            plt.plot(x2_norm, y2_norm, 'o-', label='Arista 2', linewidth=2)
        ...
        if plot:
            for i in range(n_samples):
                plt.plot([x1_norm[i], x2_norm[i]], [y1_norm[i], y2_norm[i]], 'r-', alpha=0.3)
