...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        # Alineación óptima por traslación: centroides coinciden
        tx = np.mean(x1) - np.mean(x2)
        ty = np.mean(y1) - np.mean(y2)
        x2 = x2 + tx
        y2 = y2 + ty
        
        # Buscar el mejor desplazamiento circular de índices
        # Esto alinea los puntos correspondientes a lo largo del contorno
        best_error = np.inf
        best_shift = 0
        
        for shift in range(n_samples):
            x2_shifted = np.roll(x2, shift)
            y2_shifted = np.roll(y2, shift)
            error = np.sum((x1 - x2_shifted) ** 2 + (y1 - y2_shifted) ** 2)
            if error < best_error:
                best_error = error
                best_shift = shift
        
        x2 = np.roll(x2, best_shift)
        y2 = np.roll(y2, best_shift)
        
        # Recalcular traslación óptima después del shift
        tx = np.mean(x1) - np.mean(x2)
        ty = np.mean(y1) - np.mean(y2)
        x2 = x2 + tx
        y2 = y2 + ty
