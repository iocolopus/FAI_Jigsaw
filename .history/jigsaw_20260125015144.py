...
class  Backtrack_solver():
    ...
    def solve(self):
        ...
        def r_solve(current_solution):
            ...
            if len(current_solution) == 15:
                # comprobamos que la ultima pieza encaje con la primera
                first_piece_index, first_rot = current_solution[0]
                last_piece_index, last_rot = current_solution[-1]

                dif = (spiral_index[0][0] - spiral_index[-1][0], spiral_index[0][1] - spiral_index[-1][1])
                mapping_dif_to_edge_id = {
                    (0, 1): (1,3),
                    (1, 0): (0, 2),
                    (0, -1): (3, 1),
                ...
                dissim = self.pieces[last_piece_index].edges[
                    (last_rot + first_edge_id) % 4
                ].dissimilarity(
                    self.pieces[first_piece_index].edges[
                        (first_rot + second_edge_id) % 4
                    ], c1=0.5, n_samples=50, plot=False
                )
