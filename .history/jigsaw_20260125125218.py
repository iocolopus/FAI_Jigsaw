...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        # Alineación óptima: encontrar la traslación (tx, ty) que minimiza el error
        # Solución analítica: el óptimo es cuando los centroides coinciden
        # tx* = mean(x1) - mean(x2), ty* = mean(y1) - mean(y2)
