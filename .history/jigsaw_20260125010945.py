    ...
    def solve(self):
        ...
        def r_solve(current_solution):
            ...
            if len(current_solution) < 16:
                ...
                # Determinar las piezas y rotaciones ya usadas
                used_pieces = {s[0] for s in current_solution}
                ...
                for i, pieza in enumerate(self.pieces):
                    ...
                    for rot in range(4):
                        ...
                        # Comprobar compatibilidad con aristas ya colocadas
                        score = 0
                        # Arista superior
                        if next_i > 0:
                            above_index = spiral_index[(next_i - 1) * 5 + next_j]
                            above_piece_index, above_rot = current_solution[(next_i - 1) * 5 + next_j]
                            above_edge = self.pieces[above_piece_index].edges[(above_rot + 2) % 4]
                            score += edge.dissimilarity(above_edge)

                        # Arista izquierda
                        if next_j > 0:
                            left_index = spiral_index[next_i * 5 + (next_j - 1)]
                            left_piece_index, left_rot = current_solution[next_i * 5 + (next_j - 1)]
                            left_edge = self.pieces[left_piece_index].edges[(left_rot + 1) % 4]
                            score += edge.dissimilarity(left_edge)
