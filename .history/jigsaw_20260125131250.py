...
class Edge():
    ...
    def resample_contour_uniform(self, arc_step=5, plot=False):
        """
        Remuestra el contorno con un paso de arco FIJO.
        
        Args:
            arc_step: Distancia de arco entre cada muestra (default 5 píxeles)
            
        Returns:
            Tuple de (x_resampled, y_resampled, total_arc_length)
        """
        ...
        # Generar muestras con paso de arco FIJO
        s_resampled = np.arange(0, total_arc_length, arc_step)
        ...
        if plot:
            ...
            plt.plot(x_resampled, y_resampled, 'o', label=f'Remuestreado (paso={arc_step})', color='green')
            plt.title(f'Total arc length: {total_arc_length:.1f}, Samples: {len(s_resampled)}')
            plt.axis('equal')
            plt.legend()
            plt.show()
        
        return x_resampled, y_resampled, total_arc_length

    def dissimilarity(self, other : Piece, c1 = 0, arc_step=5, plot=False):
        ...
        Usa paso de arco fijo y compara solo hasta donde lleguen ambos contornos.
        ...
        Args:
            ...
            arc_step: Paso de arco fijo para el muestreo.
        ...
        x1, y1, arc_len1 = self.resample_contour_uniform(arc_step=arc_step)
        x2, y2, arc_len2 = other.resample_contour_uniform(arc_step=arc_step)
        
        # Comparar solo hasta donde lleguen ambos (el más corto)
        n_compare = min(len(x1), len(x2))
        x1 = x1[:n_compare]
        y1 = y1[:n_compare]
        x2 = x2[:n_compare]
        y2 = y2[:n_compare]
        ...
        mean_root_square_error = (np.sum((x1 - x2) ** 2 + (y1 - y2) ** 2) / n_compare) ** 0.5
        ...
        if plot:
            plt.plot(x1, y1, 'o-', label=f'Arista 1 (arc={arc_len1:.0f})', linewidth=2)
            plt.plot(x2, y2, 'o-', label=f'Arista 2 (arc={arc_len2:.0f})', linewidth=2)
            plt.title(f'Dissimilarity: {dissimilarity_value:.2f} (n={n_compare}, tx={best_tx:.1f}, ty={best_ty:.1f})')
            plt.axis('equal')
            plt.legend()
            plt.show()
            ...
            if len(current_solution) < 16:
                ...
                def dissimilarity_key(id_pieza, edge_id):
                    return self.pieces[id_pieza].edges[edge_id].dissimilarity(
                        ...
                        ], c1=c1, arc_step=arc_step, plot=False
                    )
