        ...
        def r_solve(current_solution):
            ...
            if len(current_solution) < 15:
                ...
                for pieza_index, pieza in enumerate(self.pieces):
                    ...
                    # comprobamos que la pieza tenga al menos un borde del tipo requerido
                    is_e
                    for rot in range(4):
                        edge = pieza.edges[rot]
                        if edge.kind != 'plano' and edge.kind != pieza.edges[second_edge_id].kind:
                            edge_options.append( (rot, edge) )
