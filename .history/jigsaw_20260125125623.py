...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        Utiliza remuestreo uniforme y alineación por el pico más prominente.
        ...
        # Encontrar el pico más prominente (punto con mayor Y) de cada contorno
        peak_idx1 = np.argmax(y1)
        peak_idx2 = np.argmax(y2)
        
        # Alinear los contornos por sus picos:
        # 1. Desplazar índices para que el pico esté en la misma posición
        shift = peak_idx1 - peak_idx2
        x2 = np.roll(x2, shift)
        y2 = np.roll(y2, shift)
        
        # 2. Trasladar para que los picos coincidan en posición espacial
        tx = x1[peak_idx1] - x2[peak_idx1]
        ty = y1[peak_idx1] - y2[peak_idx1]
        x2 = x2 + tx
        y2 = y2 + ty
        ...
        if plot:
            ...
            # Marcar los picos
            plt.plot(x1[peak_idx1], y1[peak_idx1], 'b*', markersize=15, label='Pico 1')
            plt.plot(x2[peak_idx1], y2[peak_idx1], 'r*', markersize=15, label='Pico 2')
