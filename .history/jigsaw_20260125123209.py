...
class Edge():
    ...
    def _align_and_calculate_error(self, x1, y1, x2, y2, offset):
        """
        Calcula el error de alineación desplazando x2, y2 por un offset en puntos.
        Usa interpolación cíclica para manejar el desplazamiento alrededor del contorno.
        """
        n = len(x2)
        # Desplazar índices de forma cíclica
        indices = np.arange(n)
        indices_shifted = (indices + offset) % n
        
        x2_shifted = x2[indices_shifted]
        y2_shifted = y2[indices_shifted]
        
        error = np.sum((x1 - x2_shifted) ** 2 + (y1 - y2_shifted) ** 2)
        return error
    
    def _find_optimal_alignment(self, x1, y1, x2, y2):
        """
        Encuentra el mejor desplazamiento (offset) entre dos contornos remuestreados.
        Prueba todos los posibles desplazamientos y retorna el que minimiza el error.
        """
        n_samples = len(x1)
        errors = []
        ...
        for offset in range(n_samples):
            error = self._align_and_calculate_error(x1, y1, x2, y2, offset)
            errors.append(error)
        
        # Encontrar el desplazamiento óptimo
        best_offset = np.argmin(errors)
        min_error = errors[best_offset]
        
        # Aplicar el mejor desplazamiento
        indices = np.arange(n_samples)
        indices_shifted = (indices + best_offset) % n_samples
        x2_aligned = x2[indices_shifted]
        y2_aligned = y2[indices_shifted]
        
        return x2_aligned, y2_aligned, min_error
    
    def _apply_translation_alignment(self, x1, y1, x2, y2):
        """
        Alinea los contornos aplicando una traslación para minimizar la distancia.
        Calcula los centroides y los alinea.
        """
        centroid1_x = np.mean(x1)
        centroid1_y = np.mean(y1)
        centroid2_x = np.mean(x2)
        centroid2_y = np.mean(y2)
        
        tx = centroid1_x - centroid2_x
        ty = centroid1_y - centroid2_y
        
        x2_translated = x2 + tx
        y2_translated = y2 + ty
        
        return x2_translated, y2_translated

    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False, alignment_method='optimal'):
        # c1: regula la importancia que se le da en la disimilaridad la diferencia de longitudes entre aristas
        # alignment_method: 'none', 'optimal', 'translation', o 'hybrid'
        """
        Calcula una medida de disimilitud entre esta arista y otra con alineación optimizada.
        Soporta múltiples métodos de alineación para encontrar la mínima distancia posible.
        ...
        Args:
            ...
            n_samples: Número de puntos para remuestreo.
            plot: Si True, visualiza los contornos alineados.
            alignment_method: Método de alineación:
                - 'none': Sin alineación (métodoanterior)
                - 'optimal': Desplazamiento cíclico óptimo (prueba todos los offsets)
                - 'translation': Alineación por traslación de centroides
                - 'hybrid': Combina traslación + desplazamiento óptimo
        ...
        # Aplicar método de alineación seleccionado
        if alignment_method == 'optimal':
            x2, y2, _ = self._find_optimal_alignment(x1, y1, x2, y2)
        elif alignment_method == 'translation':
            x2, y2 = self._apply_translation_alignment(x1, y1, x2, y2)
        elif alignment_method == 'hybrid':
            # Primero traslación, luego desplazamiento óptimo
            x2, y2 = self._apply_translation_alignment(x1, y1, x2, y2)
            x2, y2, _ = self._find_optimal_alignment(x1, y1, x2, y2)
        # 'none' no aplica ninguna alineación adicional
        ...
        if plot:
            ...
            plt.title(f'Dissimilarity: {dissimilarity_value:.2f} (Método: {alignment_method})')
            plt.axis('equal')
            plt.legend()
            plt.show()
        ...
        if plot:
            ...
            plt.title(f'Diferencias entre puntos remuestreados (Método: {alignment_method})')
            plt.axis('equal')
            plt.show()
            ...
            if len(current_solution) < 16:
                ...
                def dissimilarity_key(id_pieza, edge_id):
                    return self.pieces[id_pieza].edges[edge_id].dissimilarity(
                        ...
                        ], c1=c1, n_samples=n_samples, plot=False, alignment_method='hybrid'
                    )
