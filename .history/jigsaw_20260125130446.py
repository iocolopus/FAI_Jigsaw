...
class Edge():
    ...
    def resample_contour_uniform(self, n_samples=50, plot=False):
        """
        Remuestra el contorno de la arista de forma uniforme usando splines cúbicos 
        parametrizados por longitud de arco NORMALIZADA [0, 1].
        
        Args:
            n_samples: Número de puntos a generar (default 50)
            
        Returns:
            Tuple de (x_resampled, y_resampled) con n_samples puntos cada uno, 
            uniformemente distribuidos en longitud de arco normalizada.
        """
        ...
        # NORMALIZAR longitud de arco a [0, 1]
        total_arc_length = arc_lengths[-1]
        arc_lengths_normalized = arc_lengths / total_arc_length
        
        # Extraer coordenadas x e y
        x_orig = contour[:, 0]
        y_orig = contour[:, 1]
        
        # Crear splines parametrizados por longitud de arco NORMALIZADA
        spline_x = interp1d(arc_lengths_normalized, x_orig, kind='linear')
        spline_y = interp1d(arc_lengths_normalized, y_orig, kind='linear')
        
        # Generar n_samples puntos uniformemente distribuidos en [0, 1]
        s_resampled = np.linspace(0, 1, n_samples)
        
        # Evaluar los splines en los nuevos puntos de parámetro normalizado
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        # Búsqueda en un rango amplio con paso fino
        ...
        # Refinamiento con paso más fino alrededor del mejor encontrado
