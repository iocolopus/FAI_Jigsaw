...
class Edge():
    ...
    def dissimilarity(self, other : Piece, c1 = 0, n_samples=50, plot=False):
        ...
        # Alinear centroides (traslación)
        tx = np.mean(x1) - np.mean(x2)
        ty = np.mean(y1) - np.mean(y2)
        x2 = x2 + tx
        y2 = y2 + ty
        
        # Buscar el mejor desplazamiento cíclico que minimiza el error
        best_error = np.inf
        best_x2, best_y2 = x2, y2
        
        for offset in range(n_samples):
            indices_shifted = (np.arange(n_samples) + offset) % n_samples
            ...
            if error < best_error:
                best_error = error
                best_x2, best_y2 = x2_shifted, y2_shifted
